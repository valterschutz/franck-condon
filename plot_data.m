...
% % subplot(1,2,1)
% plot(data(:,1),data(:,2))
% title("Measured intensity of iodine fluorescence vs wavelength", 'fontsize',16)
% xlabel("Wavelength [nm]")
% grid on
% ylabel("Intensity")
% xticks((5.5:0.1:8)*1e-7)
% xticklabels((5.5:0.1:8)*100)
% axis([5.8e-7 8e-7 0 0.25])
...
% subplot(1,2,1)
plot(data(:,1),data(:,2))
title("Hyperfine structure of a selected transition", 'fontsize',16)
xlabel("Wavelength [nm]")
ylabel("Intensity")
grid on
yticklabels([])
xt = xticks;
xticklabels(xt*1e9)
exportgraphics(gcf,'images/hyperfine.png')
% axis([5.8e-7 8e-7 0 0.25])
